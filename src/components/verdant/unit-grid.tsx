...
const PAYMENT_PLAN: PaymentStep[] = [
  ...
  { label: 'Mensais',       pct: 0.10, count: 6 },
...
export default function UnitGrid() {
  ...
  const prumadas = [...new Set(filtered.map(u => u.prumada))].sort((a, b) => Number(a) - Number(b));
  const getUnit = (floor: number, prumada: string) => filtered.find(u => u.floor === floor && u.prumada === prumada);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 flex-wrap">
        {setores.map(s => (
          <button key={s} onClick={() => setActiveSetor(s)}
            className={cn('px-4 py-2 rounded-full text-sm font-medium border transition-colors',
              activeSetor === s ? 'bg-primary text-primary-foreground border-primary'
          ...
          <DialogContent className="sm:max-w-sm">
            <DialogHeader>
              <DialogTitle className="font-display text-xl text-primary">
                Unidade {selected.code} — {selected.floor === 0 ? 'Térreo' : `${selected.floor}º andar`}
              </DialogTitle>
            </DialogHeader>
