...
import { units, type Unit } from '@/lib/orbitale-data';
...
const PAYMENT_PLAN: PaymentStep[] = [
  { label: 'Entrada',       pct: 0.40, count: 1 },
                  ...
                  return (
                    <td key={p} className="px-1 py-0.5">
                      <button
                        ...
                        onClick={() => { if (unit.status !== 'sold') { setSelected(unit); fetch('/api/track-unit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product: 'ORBITALE', unitCode: unit.code }) }).catch(() => {}) } }}
            ...
            <div className="space-y-3 text-sm">
              ...
              {manager && (
                <Button className="w-full mt-2" onClick={() => {
                  trackClick(manager.slug, 'ORBITALE')
                  window.open(`https://wa.me/${manager.phone}?text=${encodeURIComponent(`Olá ${manager.name}! Tenho interesse no Apto ${selected.code} do ORBITALE.`)}`, '_blank')
                }}>
