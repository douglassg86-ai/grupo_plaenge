...
function InterestButton({ manager, dacaoId }: { manager: ManagerType | null; dacaoId: string }) {
  ...
  if (manager) {
    return (
      <a
        ...
        onClick={() => fetch('/api/track-dacao', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dacaoId }) }).catch(() => {})}
      >
      ...
      >
        ...
        <div style={{ direction: 'ltr' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <InterestButton manager={isClientePage ? null : manager} dacaoId={property.id} />
          </div>
        </div>
      </div>
