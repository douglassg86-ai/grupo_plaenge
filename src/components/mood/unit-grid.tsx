                  ...
                  return (
                    <td key={p} className="px-1 py-0.5">
                      <button
                        ...
                        onClick={() => { if (unit.status !== 'sold') { setSelected(unit); fetch('/api/track-unit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product: 'MOOD', unitCode: unit.code }) }).catch(() => {}) } }}
