...
const PAYMENT_PLAN: PaymentStep[] = [
  ...
  { label: 'Mensais',       pct: 0.10, count: 13 },
  { label: 'Reforços',      pct: 0.15, count: 2  },
                  ...
                  return (
                    <td key={p} className="px-1 py-0.5">
                      <button
                        ...
                        onClick={() => { if (unit.status !== 'sold') { setSelected(unit); fetch('/api/track-unit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product: 'YUNA', unitCode: unit.code }) }).catch(() => {}) } }}
