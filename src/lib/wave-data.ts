...
const lotData = {
    ...
    'Quadra C': {'C L1': { price: '622.050,09', area: '341,32', type: 'CANTO/LAGO' }, 'C L2': { price: '510.659,90', area: '280,20', type: 'CANTO/LAGO' }, 'C L4': { price: '567.904,11', area: '311,61', type: 'LAGO' }, 'C L5': { price: '554.363,05', area: '304,18', type: 'LAGO' }},
    'Quadra D': {},
    ...
    'Quadra F': {'F L7': { price: '625.880,51', area: '290,20', type: 'LAGO' }, 'F L8': { price: '633.709,41', area: '293,83', type: 'LAGO' }, 'F L10': { price: '671.214,79', area: '311,22', type: 'LAGO' }, 'F L11': { price: '678.396,67', area: '314,55', type: 'LAGO' }, 'F L12': { price: '679.367,20', area: '315,00', type: 'LAGO' }, 'F L21': { price: '670.416,81', area: '310,85', type: 'LAGO' }},
...
export const lots: Lot[] = Object.entries(blockTotals).flatMap(([blockName, total]) => {
    ...
    return Array.from({ length: total }, (_, i) => {
        ...
        if(shortBlock === 'A' && lotNum === 12) { status = 'negotiation'; }
        if(shortBlock === 'C' && lotNum === 9) { status = 'negotiation'; }
        if(shortBlock === 'F' && lotNum === 20) { status = 'negotiation'; }
        ...
        if (availableData) {
            ...
            if(shortBlock === 'A' && lotNum === 12) { price = 534083.29; area = 342.39; type = 'SECO' }
            if(shortBlock === 'C' && lotNum === 9) { price = 502695.64; area = 275.83; type = 'LAGO'}
            if(shortBlock === 'F' && lotNum === 20) { price = 500592.79; area = 261.00; type = 'LAGO'}
